...
schedule_task() ->
    Task = achlys:declare(mytask, all, single, fun() ->
        ...
        achlys_view_tree:start_link(),
        achlys_view_tree:add_listener(fun(Results) ->
            io:format("Reduce : ~p~n", [Results])
        end),

        achlys_view_tree:add_variable(Set, 
            fun(Variable) -> % Map function
        ...
        achlys_view_tree:set_reduce(fun(V1, V2) -> % Reduce function
            case V1 of #{n := N1, success := S1} ->
                case V2 of #{n := N2, success := S2} -> #{
                    n => N1 + N2,
                    success => S1 + S2
